...
The flag is not needed to run a check again after collecting its previous result.
For example, an uncached check that takes 30 seconds and is polled every 10
seconds runs once during that interval. After a poll collects its result, the
following poll starts a new execution; intervening polls do not queue extra jobs.
